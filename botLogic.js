...
async function handleIncomingMessage(phone, text) {
    ...
    if (forceMenuKeywords.includes(text.toLowerCase())) {
        ...
        await whatsappService.sendMessage(phone, "Oi! Aqui é o assistente inteligente do Jean. Como você se chama?");
    ...
    switch (state) {
        ...
        case STATES.ASK_NAME:
            await whatsappService.sendMessage(phone, "Olá. Eu sou o assistente virtual do Jean.");
            await whatsappService.sendMessage(phone, "Para iniciarmos o atendimento, por favor, me informe o seu nome.");
            await updateSession(phone, { state: STATES.WAITING_NAME });
            break;
        ...
        case STATES.WAITING_NAME:
            ...
            await whatsappService.sendMessage(phone, `Muito prazer, *${username}*. Como posso auxiliá-lo(a) hoje?`);
            break;
        ...
        default:
            // Interação com a IA
            if (!ai) {
                await whatsappService.sendMessage(phone, "Minha inteligência artificial ainda não foi ativada. Peça ao Jean para adicionar a chave `GEMINI_API_KEY` no servidor para eu começar a conversar com você! 🤖");
                return;
            }
            ...
            try {
                ...
                await whatsappService.sendMessage(phone, "Vish, deu algum erro na minha rede neural e não consegui processar sua mensagem agora... Tenta de novo mais tarde? 🔌🤖");
            }
            break;
    }
}
